...
export default class ProductDetail extends Component {
    ...
    async toggleNested(id) {
        ...
        if (id) {
            let category = await axios.get(`${process.env.REACT_APP_API_URL}/categories/${id}`)
    ...
    async updateCategory(e) {
        this.toggleNested();
        const update = await axios.patch(`${process.env.REACT_APP_API_URL}/categories/${this.state.category.id}`, { name: this.state.name })
    ...
    async confirmDropCategory(id) {
        const drop = await axios.delete(`${process.env.REACT_APP_API_URL}/categories/${this.state.id}`)
        if (drop.status === 200) {
