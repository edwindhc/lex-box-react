...
class ProductDetail extends Component {
    ...
    async getProduct() {
        const product = await axios.get(`${process.env.API_APP}/products/${this.props.match.params.id}`)
        const categories = await axios.get(`${process.env.API_APP}/categories?perPage=0`)
        this.setState({
    ...
    async updateProduct() {
        ...
        const update = await axios.patch(`${process.env.API_APP}/products/${this.state.product.id}`, {
            title,
            model,
            sku,
            price,
    ...
    async onConfirmDrop() {
        const drop = await axios.delete(`${process.env.API_APP}/products/${this.state.product.id}`)
        if (drop.status === 200) {
